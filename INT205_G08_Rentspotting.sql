...
INSERT INTO RSStaff(StaffNo,Fname,Lname,Position,Salary,Branchno) VALUES ('RS-100','Boonlert','Kerdmaruay','owner',55000,'RSB-100');
  ...
  */
  ...
  grant select on  RSCUSTOMER to RScustomer;
    ...
    /*
            ===================================
          =========== Command for check data  ===========
            ===================================
    */

    SELECT
    c.brand,
    COUNT(r.rentno) AS numcouting
FROM
    rsrent r
    JOIN rscar c ON r.carno = c.carno
GROUP BY
    c.brand;
...
SELECT
    r.pickbranch,
    COUNT(r.pickbranch) AS pickdate
FROM
    rsrent r
    JOIN rscar c ON r.carno = c.carno
GROUP BY
...
SELECT
    ...
    c.engine,
    c.lugguage,
    COUNT(c.brand) AS rentingdate
FROM
    rscar c
    JOIN rsrent r ON c.carno = r.carno
WHERE
    r.pickdate = '07-JAN-2016'
GROUP BY
    c.brand,
    c.carsize,
    c.cartype,
...
FROM
    rscar c
    JOIN rsbranch b ON b.branchno = c.branchno
WHERE
    branchname = 'RSPT-BKK';
